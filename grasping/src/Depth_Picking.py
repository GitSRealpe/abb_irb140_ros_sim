...
def predict(depth, process_depth=True, crop_size=crop_size, out_size=crop_size, depth_nan_mask=None, crop_y_offset=0, filters=(5.0, 3.0, 2.0)):
    if process_depth:
    ...
    depthn = depth.copy()
    if cajon==1 or cajon==4:
       depthn[35   :230,0:300] = depth[30,150]
       depth = depth - depthn -depth[30,150]
    elif cajon==2 or cajon==5 :
       depthn[55:260,0:320] = depth[265,160]
       depth = depth - depthn -depth[265,160]
    elif cajon==3 or cajon==6:
...
class image_converter:
    ...
    #            depthfin[x,y]=0.0705
        ...
        grasps = grasp_picking.detect_grasps(points_out, ang_out, 0.5, width_img=width_out, no_grasps=5)
        for g in grasps:
	    corners=[]
	    cy,cx = g.center	
	    a = g.angle
	...
	#imagen RGB
        ...
        #cv2.imwrite('cajon3.png', rgbfin1)
