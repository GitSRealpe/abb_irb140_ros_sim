...
def detect_grasps(q_img, ang_img, threshold, width_img=None, no_grasps=5):
    ...
    if grasps == []:
        ...
        #max_pixel = np.array(np.unravel_index(np.argmax(q_img), q_img.shape))
        #grasp = Grasp(max_pixel, ang_img[max_pixel[0], max_pixel[1]])
        #grasp.length = width_img[max_pixel[0], max_pixel[1]]
